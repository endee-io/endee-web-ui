...
export async function GET(request: Request) {
  try {
    ...
    const tokens = await (await getImpersonatedClient(request, db)).listMyTokens()
...
export async function POST(request: Request) {
  try {
    ...
    const result = await (await getImpersonatedClient(request, db)).createMyToken(name.trim(), token_type)
