...
export default function TutorialsPage() {
    ...
    const [expandedSteps, setExpandedSteps] = useState<Set<string>>(
        new Set(['create-index', 'insert-vectors', 'search-vectors']),
    );
    ...
    const tutorialSteps: TutorialStep[] = [
        ...
        },
        {
            ...
            description:
                'Insert one or more vectors into an index with optional metadata and filters.',
        ...
        },
        {
            ...
            title: 'Rebuild Index',
            description:
                'Rebuild the HNSW graph for an index with new M and/or ef_construction parameters. Returns 202 immediately — the rebuild runs in the background. At least one parameter must change.',
        ...
        },
        {
            ...
            description:
                'Check the current rebuild status for an index. Returns status: idle | in_progress | completed | failed, along with timestamps and progress info.',
        ...
        },
        {
            ...
            description:
                'Retrieve a specific vector by its unique identifier. For hybrid indexes, the response also includes sparse vector data (sparseIndices and sparseValues) when available.',
        ...
        },
        {
            ...
            title: 'Create Backup',
            description:
                'Asynchronously create a backup of an index. The backup runs in the background — check /api/v1/backups/active to monitor progress.',
            ...
            run: async (payload, idx) => {
                ...
                try {
                    ...
                    const response = await fetch(
                        `/api/v1/index/${encodeURIComponent(idx)}/backup`,
                        {
                            method: 'POST',
                            headers: {
        ...
        },
        {
            ...
            description:
                'Restore a backup to a new index. Provide the backup name and target index name.',
            ...
            run: async (payload) => {
                ...
                try {
                    ...
                    const response = await fetch(
                        `/api/v1/backups/${encodeURIComponent(backup_name)}/restore`,
                        {
                            method: 'POST',
                            headers: {
                                'Content-Type': 'application/json',
                    ...
                    return {
                        success: true,
                        result: `Backup restored to index "${target_index_name}"`,
                    };
                } catch (e) {
                    return { success: false, result: `${e}` };
                }
        ...
        },
        {
            ...
            run: async (payload) => {
                ...
                try {
                    ...
                    const response = await fetch(
                        `/api/v1/backups/${encodeURIComponent(backup_name)}`,
                        {
                            method: 'DELETE',
                            headers: { ...(token && { Authorization: token }) },
                        },
                    ...
                    return {
                        success: true,
                        result: `Backup "${backup_name}" deleted successfully`,
                    };
                } catch (e) {
                    return { success: false, result: `${e}` };
                }
        ...
        },
        {
            ...
            title: 'Upload Backup',
            description:
                'Upload a backup .tar file. This is a file upload endpoint — use the Run button to select a file. If a backup with the same name already exists, the upload will fail.',
        ...
        },
        {
            ...
            description:
                'Check if a backup is currently being created. Returns active status, backup name, and index ID when a backup is in progress.',
        ...
        },
        {
            ...
            description:
                'Retrieve metadata about a backup including original index name, parameters, size, and creation timestamp.',
            ...
            run: async (payload) => {
                ...
                try {
                    const { backup_name } = JSON.parse(payload);
                    const response = await fetch(
                        `/api/v1/backups/${encodeURIComponent(backup_name)}/info`,
                        {
                            method: 'GET',
                            headers: {
                                'Content-Type': 'application/json',
                        ...
                        >
                            ...
                            >
                                <div className="flex items-center gap-3">
                                    ...
                                    <span
                                        className={`px-2 py-0.5 text-xs font-mono rounded ${getMethodColor(step.method)}`}
                                    >
                                        {step.method}
                                    </span>
                                </div>
                                {result && (
                                    <span
                                        className={`flex items-center gap-1 text-sm ${result.success ? 'text-green-600 dark:text-green-400' : 'text-red-600 dark:text-red-400'}`}
                                    >
                                        {result.success ? (
                                ...
                                <div className="border-t border-slate-200 dark:border-slate-600 p-4 space-y-4">
                                    ...
                                    {step.requiresPayload && (
                                        <div>
                                            ...
                                            <textarea
                                                ...
                                                onChange={(e) =>
                                                    setPayload(step.id, e.target.value)
                                                }
                                                rows={Math.min(
                                                    10,
                                                    (getPayload(step).match(/\n/g) || []).length +
                                                        2,
                                                )}
                                    ...
                                    <div className="flex items-center gap-3">
                                        <button
                                            ...
                                            disabled={
                                                isRunning || (step.requiresIndex && !selectedIndex)
                                            }
                                    ...
                                    {result && (
                                        <div>
                                            <div
                                                className={`text-xs font-medium uppercase mb-1 ${result.success ? 'text-green-600 dark:text-green-400' : 'text-red-600 dark:text-red-400'}`}
                                            >
                                                {result.success ? 'Response' : 'Error'}
                                            </div>
                                            <pre
                                                className={`text-sm font-mono p-3 rounded-md overflow-x-auto ${
                                                    result.success
            ...
            <div className="mt-8 bg-slate-50 dark:bg-slate-700 border border-slate-200 dark:border-slate-600 rounded-lg p-6">
                ...
                <div className="grid md:grid-cols-2 gap-4 text-sm">
                    ...
                    <div>
                        ...
                        <ul className="space-y-1 text-slate-600 dark:text-slate-300 font-mono text-xs">
                            <li>
                                <code>$in</code> - Value in array
                            </li>
                            <li>
                                <code>$range</code> - Numerical values in range
                            </li>
                        </ul>
                    </div>
                </div>
                <div className="mt-4 p-3 bg-slate-100 dark:bg-slate-800 rounded font-mono text-xs text-slate-700 dark:text-slate-300">
                    Example:{' '}
                    <code>{`[{"year": {"$gte": 2020}}, {"type": {"$in": ["article", "paper"]}}]`}</code>
                </div>
            </div>
        </div>
    );
