 ...
 *
 * APP_MODE:
 *   - "single-server": one Endee server, its URL + root token come from the
 ...
 *   - "multi-server": servers are user-managed and persisted to a JSON file on
...
export function getAppMode(): AppMode {
  return process.env.APP_MODE === "multi-server" ? "multi-server" : "single-server"
}

/** Absolute-or-relative path to the multi-server-mode server list file. */
